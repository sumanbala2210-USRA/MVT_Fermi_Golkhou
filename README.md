...
⚠️ **Place this `config_MVT.yaml` (`config_MVT_general.yaml`) file in the directory from where you will run the `MVTfermi` (`MVTgeneral`) script or `mvtfermi` (`mvtgeneral()`) function . Update paths to reflect your local setup.** ⚠️
...
- The `full_grb_time_lo_edge` (or `'time_edges'` array) should contain data between **30s before the T0** and **T0+2×T90+20s** [i.e., T0−30s to T0+2×T90+20s] of data.
