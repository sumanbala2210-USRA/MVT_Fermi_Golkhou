...
from .find_opt_int import find_optimum_int, convert_res_coarse, ExponentialFloat
...
def process_iteration(params):
    try:
        trigger_number, grb_range, grb_count, bkg_range, bkg_count, tt1, tt2, bw, N, f1, f2, k, path, all_fig  = params
        return find_optimum_int(trigger_number, grb_range, grb_count, bkg_range, bkg_count, tt1, tt2, bw, N, f1, f2, k, path, all_fig )
...
def compute_grb_time_bounds_int(T0, T90, delta, start_padding=False, end_padding=False, end_t90=2.0):
    """
    Compute the widest time range needed for all delta values, using delt_max.
...
def evolve_optimum_int(trigger_number,en, time_edges, counts, back_edges, back_counts, T0, T90, start_padding, end_padding, delt, bw, N, cores, f1=5,f2=3, path = None, tr_fixed=None, all_fig=False):
    ...
    grb_start = int((T0 - time_edges[0]) / bw)+1
    ...
    for k in range(nn):
        ...
        if tt2 > T0 + T90/0.9:# + ((delt+2) * end_padding) or tt2 + delt > max(time_edges):
            break
        tr[k] = tt2
        ...
        #exit()
        tasks.append((trigger_number, grb_range, grb_count, bkg_range, bkg_count, tt1, tt2, bw, N, f1, f2, k, path, all_fig))
    #exit()
    ...
    significance_min_z = 1
    significance_min_weighted = 1
