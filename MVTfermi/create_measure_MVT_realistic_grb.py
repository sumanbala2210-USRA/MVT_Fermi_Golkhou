...
def _run_full_simulation_worker(params: Dict[str, Any]) -> Tuple[float, float]:
    ...
    fig.suptitle(f'GRB Simulation Factor: {round(params["factor"], 3)}', fontsize=14)
...
if __name__ == '__main__':
    ...
    output_dir = Path(f"grb_run")#_{timestamp}")
    ...
    for fact in values:
        ...
        plt.savefig(output_dir / f"representative_{simulation_params['factor']}_model.png")
        plt.close()
