...
def get_best_detectors(trigger_number, trigger_dir):
    """
    Get the best detectors for a given trigger number from the trigger directory.

    Args:
    trigger_number (str): The trigger number to filter detectors.
    trigger_dir (str): The directory where the trigger files are located.

    Returns:
    list: A list of best detector names.
    """
    # Initialize TriggerFtp with the trigger directory
    ftp = TriggerFtp(trigger_dir)
    
    # Get the best detectors for the specified trigger number
    best_detectors = ftp.get_best_detectors(trigger_number)
    
    return best_detectors

def trigger_file_list(trigger_dir, file_type, trigger_number, nai_dets='all'):
    """
    ...
    if nai_dets == 'all':
        pattern = os.path.join(trigger_dir, f"glg_{file_type}_*_bn{trigger_number}*.fit")
        file_abs_path_list = glob.glob(pattern)
    else:
        for det in nai_dets:
            pattern = os.path.join(trigger_dir, f"glg_{file_type}_{det}_bn{trigger_number}*.fit")
            file_abs_path_list.extend(glob.glob(pattern))
...
def trigger_process_old(
    ...
    try:
        ...
        tte_list_path.sort()
        print("TTE files found:", tte_list_path)
...
def trigger_process(
    ...
    """
    try:
        file_name = file_write+'.npz'
        file_write_path = os.path.join(trigger_directory, file_name)
        print(f"Generating and saving data to \n{file_name}")
        energy_range_nai = (en_lo, en_hi)
        trange_bkg = (bkgd_range[0][0] - 10, bkgd_range[1][1] + 10)
        trange = (tt1 - 20, tend + 20)
