...
class GbmSimulationTask_v2(BaseSimulationTask):
    """Task for Fermi GBM light curves using specified pulse shapes."""
    def run(self):
        # --- NEW: Define pulse-specific keys for easy separation ---
        pulse_specific_keys = {
            'gaussian': ['sigma', 'center_time'],
            'triangular': ['width', 'center_time', 'peak_time_ratio'],
            'norris': ['rise_time', 'decay_time', 'start_time'],
        ...
        keys_to_remove = ['type', 'en_lo', 'en_hi', 'pulse_shape', 'bkgd_times', 'pulse_configs', 'bin_width', 'center_time', 't_start', 't_stop', 'peak_time_ratio', 'start_time']
        try:
            ...
            #pulse_params = {k: self.params[k] for k in pulse_specific_keys.get(pulse_shape, []) if k in self.params}
            pulse_params = {k: round(self.params[k], 3) if isinstance(self.params[k], (int, float)) else self.params[k] 
                            for k in pulse_specific_keys.get(pulse_shape, []) if k in self.params}
            ...
            results = haar_power_mod(counts, np.sqrt(counts), min_dt=min_dt, doplot=True, afactor=-1., file=mvt_plot_path, verbose=False)

            final_results = {
                'pulse': pulse_shape,
                'peak_amplitude': self.params.get('peak_amplitude'),
                'background_level': self.params.get('background_level'),
                # Add trigger info if it exists
                'trigger_number': self.params.get('trigger_number'),
                'det': self.params.get('det'),
                'angle': self.params.get('angle'),
                # Add results
            ...
            final_results.update(pulse_params)
            ...
            all_pulse_keys = set(key for sublist in pulse_specific_keys.values() for key in sublist)
            for key in all_pulse_keys:
                error_results[key] = None # Will become NaN in the DataFrame
            error_results.update(pulse_params)
            for key in keys_to_remove:
...
# NEW TASK CLASS FOR GBM
class GbmSimulationTask1(BaseSimulationTask):
    """Task for Fermi GBM light curves using specified pulse shapes."""
    def run(self):
        keys_to_remove = ['type', 'en_lo', 'en_hi', 'pulse_shape', 'bkgd_times', 'pulse_configs', 'bin_width', 'center_time', 't_start', 't_stop']
        try:
            # ... (the main 'try' block code is correct and unchanged) ...
            # 1. Set up source pulse function
            pulse_shape = self.params.get('pulse_shape')
            ...
            final_results['pulse'] = pulse_shape  # Convert to list for JSON serialization
            ...
            final_results['pulse'] = pulse_shape
